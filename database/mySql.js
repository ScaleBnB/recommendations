...
module.exports = {
  ...
  getAllRecommendationsByListingId: (id, callback) => {
    //"SELECT recommendations.id, recommendations.title, recommendations.price,recommendations.homeType,recommendations.bedsNumber,recommendations.reviewsAverage,recommendations.numberOfReviews,recommendations.likedStatus,recommendations.plusStatus,recommendations.image1 from recommendations INNER JOIN listings ON listings.id = recommendations.listing_id WHERE listing_id = ?"
    con.query(
